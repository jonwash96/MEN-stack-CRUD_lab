...
router.delete('/:resourceType/:resourceId', async (req, res) => {
    const Resource = req.params.resourceType==='asset' ? Asset : License;
    ...
    const oldData = Object.create(data);
    await data.deleteOne();

    res.render('resources/delete-confirmation.ejs', { data:oldData, template });
})

// Update - PUT /resources/:resourceID (req.body) => Updata a speciffic resource using request body
router.put('/:resourceType/:resourceId', async (req, res) => {
...
router.get('/:resourceType/:resourceId/edit', async (req, res) => {
    ...
    res.render(`resources/${req.params.resourceType}s/edit.ejs`, { data });
})
...
router.get('/:resourceType/:resourceId', async (req, res) => {
    const Resource = Resources[req.params.resourceType.replace(/s$/, '')];
    const data = await Resource.findById(req.params.resourceId);
    const template = templates[req.params.resourceType.replace(/s$/, '')];

    res.render('resources/show.ejs', { data, template });
})
